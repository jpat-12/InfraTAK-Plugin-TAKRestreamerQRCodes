...
# Anchor on the streaming site's header comment + host-open line. Tried in
# order: a hand-added "TAK Video Restreamer" block (confirmed the real shape
# on ILWG's server — tak-video-restreamer's own Flask app fronted directly by
# Caddy, not generated by infra-TAK's generate_caddyfile()) falling back to
# infra-TAK's generic "MediaMTX Web Console" block for setups that use that
# instead. Either way we don't need to know the literal host.
ANCHOR_PATTERNS = [
